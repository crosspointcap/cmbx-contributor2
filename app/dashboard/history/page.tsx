...
const TRANCHES = ['AAA', 'AS', 'AA', 'A', 'BBB-', 'BB'] as const
...
const hygVixOptions = {
  ...
  scales: {
    ...
    y:  { ...baseChartOptions.scales.y, position: 'left'  as const, title: { display: true, text: 'HYG', color: '#eebb00', font: { size: 9 } } },
    y1: { ...baseChartOptions.scales.y, position: 'right' as const, title: { display: true, text: 'VIX', color: '#bb55ee', font: { size: 9 } }, grid: { drawOnChartArea: false } },
  },
}
...
export default function HistoryPage() {
  ...
  // Filters
  const [selectedSeries,  setSelectedSeries]  = useState('6')
  const [selectedTranche, setSelectedTranche] = useState('AAA')
  ...
  useEffect(() => {
    ...
    supabase.from('series_config').select('series_number').eq('active', true).order('sort_order')
      .then(({ data }) => {
        if (data && data.length > 0) {
          const nums = data.map((r: { series_number: string }) => r.series_number)
          setSeriesList(nums)
          setSelectedSeries(nums[0])
        }
      })
  }, [authChecked])
  ...
  useEffect(() => {
    if (!authChecked) return
    loadData()
  }, [authChecked, selectedSeries, selectedTranche, dateRange]) // eslint-disable-line react-hooks/exhaustive-deps
  ...
  async function loadData() {
    ...
    let snapshotQ = supabase
      ...
      .select('date, best_bid, best_ask, trade_count, avg_trade_px')
      .eq('series_number', selectedSeries)
      .eq('tranche_name', selectedTranche)
    ...
    // Build market context lookup: date string → row
  ...
  const cmbxChartData = useMemo(() => {
    ...
    // Map date → first trade price for that day
  ...
  const spreadVal = (s: DailySnapshot) =>
    s.best_bid != null && s.best_ask != null
      ? (s.best_ask - s.best_bid).toFixed(2)
      : null
  ...
  return (
    <div style={{ background: '#0a0a0a', color: '#ccc', fontFamily: 'Courier New, monospace', fontSize: '13px', height: '100vh', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
      ...
      {/* Filter bar */}
      <div style={{ display: 'flex', alignItems: 'center', gap: '20px', padding: '6px 12px', borderBottom: '1px solid #1e1e1e', flexShrink: 0, background: '#080808' }}>

        {/* Series */}
        <div style={{ display: 'flex', alignItems: 'center', gap: '7px' }}>
          <span style={{ color: '#3a3a3a', fontSize: '11px', letterSpacing: '1px' }}>SERIES</span>
          <select
            value={selectedSeries}
            onChange={e => setSelectedSeries(e.target.value)}
            style={{ background: '#111', color: '#ccc', border: '1px solid #252525', padding: '2px 6px', fontFamily: 'Courier New, monospace', fontSize: '13px', cursor: 'pointer', outline: 'none' }}
          >
            {seriesList.map(s => <option key={s} value={s}>CMBX.{s}</option>)}
          </select>
        </div>

        {/* Tranche */}
        <div style={{ display: 'flex', alignItems: 'center', gap: '7px' }}>
          <span style={{ color: '#3a3a3a', fontSize: '11px', letterSpacing: '1px' }}>TRANCHE</span>
          <select
        ...
        {loading && <span style={{ color: '#3a3a3a', fontSize: '11px', letterSpacing: '1px' }}>LOADING...</span>}

        <span style={{ marginLeft: 'auto', color: '#222', fontSize: '11px' }}>
          CMBX.{selectedSeries}.{selectedTranche} · {dateRange}
        </span>
      </div>
      ...
      <div style={{ flex: 1, display: 'flex', overflow: 'hidden' }}>
        ...
        <div style={{ width: '60%', display: 'flex', flexDirection: 'column', borderRight: '1px solid #1a1a1a', overflow: 'hidden' }}>
          ...
          <div style={{ flex: '0 0 45%', overflow: 'auto', borderBottom: '1px solid #1a1a1a' }}>
            <div style={{ position: 'sticky', top: 0, background: '#0c0c0c', padding: '5px 12px', borderBottom: '1px solid #1e1e1e', zIndex: 1 }}>
              ...
              <span style={{ color: '#2a2a2a', fontSize: '11px', marginLeft: '8px' }}>CMBX.{selectedSeries}.{selectedTranche}</span>
            </div>
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '13px' }}>
              <thead>
                <tr style={{ color: '#444', position: 'sticky', top: '26px', background: '#0a0a0a', zIndex: 1 } as React.CSSProperties}>
                  ...
                  <th style={{ textAlign: 'right', padding: '4px 10px', borderBottom: '2px solid #66ff88', fontWeight: 400 }}>BEST BID</th>
                  <th style={{ textAlign: 'right', padding: '4px 10px', borderBottom: '2px solid #ff6666', fontWeight: 400 }}>BEST ASK</th>
                  <th style={{ textAlign: 'right', padding: '4px 10px', borderBottom: '1px solid #1a1a1a', fontWeight: 400 }}>SPREAD</th>
                  <th style={{ textAlign: 'right', padding: '4px 10px', borderBottom: '1px solid #1a1a1a', fontWeight: 400 }}>TRADES</th>
              ...
              <tbody>
                {snapshots.length === 0 ? (
                  <tr>
                    <td colSpan={6} style={{ padding: '24px 12px', color: '#2a2a2a', textAlign: 'center' }}>
                      — no snapshot data for selected range
                    </td>
                  </tr>
                ) : snapshots.map((s, i) => {
                  const sp = spreadVal(s)
                  return (
                    <tr key={s.date} style={{ background: i % 2 === 0 ? '#0a0a0a' : '#0d0d0d', borderBottom: '1px solid #141414' }}>
                      <td style={{ padding: '4px 12px', color: '#666' }}>{fmtDateFull(s.date)}</td>
                      <td style={{ textAlign: 'right', padding: '4px 10px', color: s.best_bid  != null ? '#66ff88' : '#2a2a2a' }}>{s.best_bid  ?? '—'}</td>
                      <td style={{ textAlign: 'right', padding: '4px 10px', color: s.best_ask  != null ? '#ff6666' : '#2a2a2a' }}>{s.best_ask  ?? '—'}</td>
                      <td style={{ textAlign: 'right', padding: '4px 10px', color: sp          != null ? '#888'    : '#2a2a2a' }}>{sp          ?? '—'}</td>
                      <td style={{ textAlign: 'right', padding: '4px 10px', color: s.trade_count ? '#ccc' : '#2a2a2a' }}>{s.trade_count ?? '—'}</td>
          ...
          <div style={{ flex: 1, overflow: 'auto' }}>
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '13px' }}>
              ...
              <tbody>
                {trades.length === 0 ? (
                  <tr>
                    <td colSpan={7} style={{ padding: '24px 12px', color: '#2a2a2a', textAlign: 'center' }}>
                      — no trades for selected range
                    </td>
                  </tr>
                ) : trades.map((t, i) => (
                  <tr key={t.id} style={{ background: i % 2 === 0 ? '#0a0a0a' : '#0d0d0d', borderBottom: '1px solid #141414' }}>
                    ...
                    <td style={{ padding: '4px 8px',  color: '#fff' }}>CMBX.{t.series_number}.{t.tranche_name}</td>
                    <td style={{ textAlign: 'right', padding: '4px 8px', color: '#f0c040' }}>
                      {t.price ?? <span style={{ color: '#2a2a2a' }}>—</span>}
                    </td>
                    <td style={{ textAlign: 'right', padding: '4px 8px', color: '#777' }}>
                      {t.trade_size != null ? `${t.trade_size}MM` : <span style={{ color: '#2a2a2a' }}>—</span>}
                    </td>
                    <td style={{ textAlign: 'right', padding: '4px 8px', color: t.spx_close != null ? '#3388ff' : '#2a2a2a' }}>
                      {t.spx_close != null ? t.spx_close.toLocaleString() : '—'}
                    </td>
                    <td style={{ textAlign: 'right', padding: '4px 12px', color: t.cdx_hy_spread != null ? '#eebb00' : '#2a2a2a' }}>
                      {t.cdx_hy_spread != null ? `${t.cdx_hy_spread} bps` : '—'}
                    </td>
                  </tr>
                ))}
              </tbody>
        ...
        <div style={{ width: '40%', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
          ...
          {/* Chart 1: CMBX Bid / Offer + trade dots */}
          <div style={{ flex: 1, borderBottom: '1px solid #1a1a1a', padding: '8px 12px', display: 'flex', flexDirection: 'column', minHeight: 0 }}>
            <div style={{ flexShrink: 0, marginBottom: '4px' }}>
              ...
              <span style={{ color: '#2a2a2a', fontSize: '10px' }}> · </span>
              <span style={{ color: '#f0c040', fontSize: '10px', letterSpacing: '1px' }}>CMBX.{selectedSeries}.{selectedTranche}</span>
            </div>
            <div style={{ flex: 1, minHeight: 0, position: 'relative' }}>
              <Line data={cmbxChartData} options={baseChartOptions} />
            </div>
          </div>
          ...
          <div style={{ flex: 1, padding: '8px 12px', display: 'flex', flexDirection: 'column', minHeight: 0 }}>
            ...
            <div style={{ flex: 1, minHeight: 0, position: 'relative' }}>
              <Line data={hygVixChartData} options={hygVixOptions} />
            </div>
          </div>
        </div>
      </div>
